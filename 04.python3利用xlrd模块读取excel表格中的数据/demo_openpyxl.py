...
def readExcel(readExcelNameOrPath='', readSheets=None, contentType='json'):
    ...
    try:
        ...
        for sheetInfo in readSheetsInfoList:
            dataRows = sheetInfo.max_row + 1
            dataCols= sheetInfo.max_column + 1
            if dataRows > 0:
                temDict = []
                for row in range(1, dataRows):
                    for col in range(1, dataCols):
                        ...
                        temD['row'] = row + 1
                        temD['col'] = col + 1
                        temD['content'] = str(sheetInfo.cell(row = row, column=col).value)
                        temDict.append(temD)
                readSheetsContent[sheetInfo.title] = temDict
            else:
                readSheetsContent[sheetInfo.title] = []
